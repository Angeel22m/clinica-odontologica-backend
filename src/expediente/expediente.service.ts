...
export class ExpedienteService {
  ...
  async create(createExpedienteDto: CreateExpedienteDto) {
    ...
    try {
      ...
      console.error(
        'Error inesperado al crear expediente y/o asociación:',
        error,
      );
  ...
  async findAll() {
    return this.prisma.expediente.findMany({ include: expedienteInclude });
  }
  ...
  async findOne(id: number, idPaciente = false) {
    ...
    const signedUrls =
      await this.storageService.generateSignedUrls(allFilePaths);

    const archivosConUrls = expediente.archivos
      .map((archivo, index) => ({
        id: archivo.id,
  ...
  async getHistorialPaciente(pacienteId: number) {
    ...
    if (!expedienteDetalles.length) {
      throw new NotFoundException(
        `No se encontró historial para el paciente con ID ${pacienteId}`,
      );
    }
