 ...
 async crearFacturaPDFKit(citaId: number): Promise<Buffer> {
  ...
  const cita = await this.prisma.cita.findUnique({
    ...
    include: {
      ...
      servicio: true,
      Factura: { include: { DetalleFactura: true } }
    }
  });
  ...
  // ===========================================================
  if (cita.Factura?.id && cita.Factura) {
...
private generarPDF(factura: any, cita: any): Promise<Buffer> {
  return new Promise(resolve => {
    ...
    doc.fontSize(12)
      ...
      .text(`Fecha de emisión: ${factura.fechaEmision.toLocaleDateString()}`, 350)
    ...
    factura.DetalleFactura.forEach(item => {
      doc.fontSize(11).fillColor('black')
        .text(item.descripcion, 50, doc.y, { width: 200 })
        .text(item.cantidad.toString(), 260, doc.y)
        .text(`L. ${item.precioUnitario}`, 340, doc.y)
        .text(`L. ${item.totalLinea}`, 430)
        .moveDown(0.8);
    });
    ...
    doc.fontSize(11).fillColor('black')
      ...
      .font('Helvetica')
      .text(`L. ${factura.subtotal.toFixed(2)}`, 480)
      .text(`L. ${factura.isv15.toFixed(2)}`, 480)
      .font('Helvetica-Bold')
      .text(`L. ${factura.totalPagar.toFixed(2)}`, 480);
 ...
 async obtenerFacturaPorCitaId(citaId: number) {
  ...
  const cita = await this.prisma.cita.findFirst({
    where: {
      ...
      Factura: null            // Asegura que NO esté facturada
    },
    include: {
      paciente: true,
      doctor: { include: { persona: true } },
